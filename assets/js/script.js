...
function clickBoton() {
  ...
  if (nombreGasto && !isNaN(valorGasto)) {
	...
	showAlert('Por favor, ingrese un nombre y un valor válido para el gasto.', 'error');
  }
}
...
function actualizarListaDeGastos() {
  ...
  listaNombreGastos.forEach((elemento, posicion) => {
	...
	li.textContent = `${elemento} - USD ${valorGasto.toFixed(2)} - ${descripcionGasto}`;
  ...
  if (totalGastos > 150) {
	showAlert("¡Advertencia! El total de gastos ha superado el límite de 150.", 'error');
  }
